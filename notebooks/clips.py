...
def parse_args():
    ...
    parser.add_argument("--clip_npz", type=str, help="input: npz file with clips")
...
clips_file = f"{prefix}/{ref_id}/clips.npz"
coverage_file = f"{prefix}/{ref_id}/coverage.npz"

out_nclips = "test_fig/nclips.png"
out_nclips_df = "test_fig/nclips.csv"
out_summary = "test_fig/clip_summary.png"
out_html = "test_fig/clips.html"
out_trajs = "test_fig/clip_traj.png"
out_csv_traj = "test_fig/clip_traj.csv"

freq_thr = 0.5
cov_thr = 10
conv_window = 100
n_clips_freq_thr = 0.5
n_top_trajs = 33

Cls, Covs, samples = load_data(clips_file, coverage_file)
Covs = coverage_comparison(Covs)
Fs = safe_division(Cls, Covs, extra=np.nan)

# %%
df = n_clip_scatterplots(
    Cls, Covs, samples, conv_window, n_clips_freq_thr, savename=out_nclips
)
df.to_csv(out_nclips_df, index=False)


# %%

dF = relevant_deltaF(Fs, Covs, freq_thr, cov_thr)

plot_summary(Fs, samples, freq_thr, dF, savename=out_summary)
...
if len(df) > 0:
    ...
    plot_traj(Fs, Covs, dF, samples, idxs, freq_thr, cov_thr, savename=out_trajs)

    plotly_clips(Fs, samples, freq_thr, savename=out_html)
